...
from ai_generator import generate_mcqs, MODEL


st.set_page_config(
    page_title="AI Quiz Generator",
    page_icon="🎯",
    layout="wide",  # use the full width of the browser/projector
)

# --- Center main content with side margins for readability on wide displays ---
_, main, _ = st.columns([1, 3, 1])

with main:
    st.title("🎯 AI Quiz Generator")
    st.caption("Upload a PowerPoint and get an instant AI-generated multiple-choice quiz.")
    st.markdown(
        "**How it works** — Upload a `.pptx` deck, choose how many questions and at "
        "what difficulty, and an AI will write a quiz from your slides. Take it right "
        "here in your browser, get an instant score, and see why each wrong answer was wrong."
    )
    st.divider()

    # --- Session state setup ---
    # Streamlit re-runs the WHOLE script on every interaction, so any variable you
    # want to remember between interactions must live in st.session_state.
    ...
    # --- Footer ---
    st.divider()
    st.caption(f"Built with Streamlit · Powered by `{MODEL}` via OpenRouter")
